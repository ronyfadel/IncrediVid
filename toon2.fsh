...
float grad()
{
    vec3 left           = texture2D(input_texture, left_texture_coordinate).rgb;
	vec3 right          = texture2D(input_texture, right_texture_coordinate).rgb;
    vec3 top            = texture2D(input_texture, top_texture_coordinate).rgb;
	vec3 top_left       = texture2D(input_texture, top_left_texture_coordinate).rgb;
	vec3 top_right      = texture2D(input_texture, top_right_texture_coordinate).rgb;
	vec3 bottom         = texture2D(input_texture, bottom_texture_coordinate).rgb;
	vec3 bottom_left    = texture2D(input_texture, bottom_left_texture_coordinate).rgb;
	vec3 bottom_right   = texture2D(input_texture, bottom_right_texture_coordinate).rgb;
